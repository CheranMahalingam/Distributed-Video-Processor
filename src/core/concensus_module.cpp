...
void ConcensusModule::ElectionCallback(const int term) {
    Log(LogLevel::Info) << id_ << "election timer expired";

    if (state_ != ElectionRole::Candidate && state_ != ElectionRole::Follower) {
        Log(LogLevel::Info) << id_ << "state invalid for election";
        return;
    }

    if (current_term_ != term) {
        Log(LogLevel::Info) << id_ << "term changed from" << current_term_ << "to" << term;
        return;
    }
...
void ConcensusModule::HeartbeatCallback() {
    if (state_ != ElectionRole::Leader) {
        Log(LogLevel::Info) << id_ << "invalid state for sending heartbeat";
        return;
    }
    ...
    for (auto peer_id:peer_ids_) {
        Log(LogLevel::Info) << id_ << "sending AppendEntries call to" << peer_id;
        // Make request append entries call and get reply
        auto [reply_term, reply_success] = AppendEntries(peer_id, saved_term);
        if (!reply_success) {
            return;
        }
        Log(LogLevel::Info) << id_ << "received AppendEntries reply from" << peer_id;
    
        if (reply_term > saved_term) {
            Log(LogLevel::Info) << id_ << "term out of date in heartbeat reply, changed from" << saved_term << "to" << reply_term;
...
void ConcensusModule::StartElection() {
    ...
    int saved_term = current_term_;
    vote_ = id_;
    Log(LogLevel::Info) << id_ << "becomes Candidate, term:" << saved_term;

    int votes_received = 1;

    for (auto peer_id:peer_ids_) {
        Log(LogLevel::Info) << id_ << "sending RequestVote call to" << peer_id;
        ...
        Log(LogLevel::Info) << id_ << "received RequestVote reply from" << peer_id;

        // State changed when making calls
        if (state_ != ElectionRole::Candidate) {
            Log(LogLevel::Info) << 5; //<< id_ << "changed state while waiting for reply";
            return;
        }

        // Another server became the leader
        if (reply_term > saved_term) {
            Log(LogLevel::Info) << id_ << "term out of date, changed from" << saved_term << "to" << reply_term;
            ResetToFollower(reply_term);
            return;
            ...
            if (reply_vote_granted) {
                ...
                if (votes_received*2 > peer_ids_.size()) {
                    Log(LogLevel::Info) << id_ << "wins election with" << votes_received << "votes";
                    PromoteToLeader();
                    return;
                }
            }
    ...
    Log(LogLevel::Info) << id_ << "election was unsuccessful, restarting...";
    ElectionTimeout(current_term_);
}

void ConcensusModule::Shutdown() {
    state_ = ElectionRole::Dead;
    Log(LogLevel::Info) << id_ << "has shutdown";
}
...
std::tuple<int, bool> ConcensusModule::RequestVote(const std::string peer_id, const int term) {
    ...
    if (cq.Next(&tag, &ok) && ok && tag == (void*)1) {
        if (status.ok()) {
            ...
            Log(LogLevel::Info) << id_ << "rpc RequestVote call failed with error" << status.error_code() << status.error_message();
            return std::make_tuple(-1, false);
        }
    } else {
        Log(LogLevel::Info) << id_ << "rpc RequestVote call failed unexpectedly";
        return std::make_tuple(-1, false);
    }
}
...
std::tuple<int, bool> ConcensusModule::AppendEntries(const std::string peer_id, const int term) {
    ...
    if (cq.Next(&tag, &ok) && ok && tag == (void*)1) {
        if (status.ok()) {
            ...
            Log(LogLevel::Info) << id_ << "rpc AppendEntries call failed with error" << status.error_code() << status.error_message();
            return std::make_tuple(-1, false);
        }
    } else {
        Log(LogLevel::Info) << id_ << "rpc AppendEntries call failed unexpectedly";
        return std::make_tuple(-1, false);
    }
}
...
void ConcensusModule::PromoteToLeader() {
    ...
    Log(LogLevel::Info) << id_ << "becomes leader, term:" << current_term_;
...
void ConcensusModule::ResetToFollower(const int term) {
    ...
    current_term_ = term;
    vote_ = -1;
    Log(LogLevel::Info) << id_ << "becomes follower, term:" << current_term_;
...
void ConcensusModule::ElectionTimeout(const int term) {
    ...
    Log(LogLevel::Info) << id_ << "new election timer created" << random_timeout;
...
void ConcensusModule::HeartbeatTimeout() {
    Log(LogLevel::Info) << id_ << "new heartbeat timer created";
